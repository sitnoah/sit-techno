...
add_filter('wp_privacy_personal_data_exporters',function($exporters){$exporters['sit-enquiries']=array('exporter_friendly_name'=>'SIT project enquiries','callback'=>'sit_core_export');return $exporters;});
function sit_core_export($email,$page=1){
    ...
    foreach($rows as $row){$fields=array();foreach(array('reference','name','email','company','goal','service','brief','budget','timeline','consent_at','status','created_at','updated_at') as $name){$fields[]=array('name'=>ucwords(str_replace('_',' ',$name)),'value'=>$row->$name);}$data[]=array('group_id'=>'sit-enquiries','group_label'=>'SIT project enquiries','item_id'=>'sit-enquiry-'.$row->id,'data'=>$fields);}
    return array('data'=>$data,'done'=>count($rows)<50);
}
add_filter('wp_privacy_personal_data_erasers',function($erasers){$erasers['sit-enquiries']=array('eraser_friendly_name'=>'SIT project enquiries','callback'=>'sit_core_erase');return $erasers;});
